    ...
    async execute(request: ExecutionRequest): Promise<ExecutionResponse> {
        ...
        try {
            ...
            if (totalTasks === 1) {
                ...
                if (!isSuccessful) {
                    const completedCondition = executionResult.conditions?.find((c: any) => c.type === 'Completed');
            ...
            const operationFinished = operation.promise().then(async ([executionResult]: [any]) => {
