...
* **CRITICAL: Delegation via Distributed MCP Swarm:** Ensure that execution is orchestrated through the native Multi-Agent Protocol (MCP) tool registry, not local shells or processes. You are coordinating an underlying distributed context via tools (like SWARM_DISPATCH, SUPERVISE_MERGE, etc).
...
* **Trust Work Phases:** Work Phases are expert tools executing asynchronously. Your role is to clearly define the objective and provide necessary inputs for the swarm to execute, not to micromanage their internal loops.
